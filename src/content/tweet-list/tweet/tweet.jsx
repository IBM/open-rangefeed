...
export default class Tweet extends React.Component {
  constructor(props) {
    ...
    this.replyCountSmall = hasReplyCountSmall === true ? this.getRand(1, 3) : '';
  }
  ...
  getNumProfileImages() {
    return require.context('../../../../public/img/faces/legacy', false, /\.(png|jpe?g|svg)$/).keys().length;
  }
  ...
  render() {
    ...
    const replyCount = mood === 'negative' ? this.replyCountLarge : this.replyCountSmall;
    const replyCountKnotation = this.toKnotation(replyCount);
    const rtCount = this.toKnotation(typeof replyCountKnotation === 'string' ? '' : Math.round(replyCountKnotation * 1.6));
    const likeCount = this.toKnotation(typeof replyCountKnotation === 'string' ? '' : Math.round(replyCountKnotation * 2.1));
    ...
    if (img && img.indexOf('data:image') !== 0) fullImgPath = `${process.env.PUBLIC_URL}/img/${img}`;
    ...
    if (user) {
      if (user.img) {
        ...
        else facePath = `${process.env.PUBLIC_URL}/img/faces/special/${user.img}`;
      }
      if (user.name) {
        userNameVisibility = '';
        usrHandle = user.name;
      ...
      >
        ...
        <div className={`right ${rightHasName}`}>
          <div className={`name ${userNameVisibility}`}>
            ...
            {
              user && user.vip === true
              && <img className="icon-verified" src={`${process.env.PUBLIC_URL}/img/verified.png`} alt="" />
            }
          </div>
          <div className="text">{children}</div>
          {
            img
            && <img className="img" src={fullImgPath} alt="" />
          }
          <div className="icons">
